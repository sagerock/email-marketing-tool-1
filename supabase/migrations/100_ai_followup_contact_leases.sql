...
BEGIN
  ...
  WITH one_per_contact AS MATERIALIZED (
    ...
    WHERE fc.status = 'in_progress'
      ...
      AND NOT EXISTS (
        SELECT 1
        FROM public.ai_followup_contacts AS leased
        WHERE leased.client_id = fc.client_id
          AND leased.contact_id = fc.contact_id
          AND leased.id <> fc.id
          AND leased.status = 'in_progress'
          AND leased.next_followup_at > statement_timestamp()
          AND leased.next_followup_at <= statement_timestamp()
