CREATE INDEX IF NOT EXISTS idx_contacts_canonical_email_suppression
...
BEGIN
  ...
  WHERE c.client_id = p_client_id
    ...
    AND NOT EXISTS (
      SELECT 1
      FROM public.contacts suppressed
      WHERE suppressed.client_id = c.client_id
        AND lower(btrim(suppressed.email)) = lower(btrim(c.email))
        AND (suppressed.unsubscribed = true OR suppressed.bounce_status = 'hard')
    )
    AND (
      p_bypass_safe_send
