-- 083: engagement_overview(client, days, wait_days) → one JSON blob for the Engagement page
--
-- Answers, per client: who arrived (and from where), who replied, who is waiting on a
-- human ("replied and nobody answered", "clicked and never contacted"), what the
-- Salesforce sample-request pipeline looks like and which opps have stalled, and who
-- is most engaged. "Touched by the client" = contacts.salesforce_last_activity_date
-- (Salesforce's rollup of logged tasks/events). "Touched by us" = outbound rows in
-- email_conversations. Server-side (service role) only; RLS on analytics_events makes
-- direct client-side aggregation impractical.
...
waiting_raw AS (
  SELECT c.id, c.email, c.first_name, c.last_name, c.company, c.source_code,
         c.last_replied_at, c.last_engaged_at, c.salesforce_last_activity_date, c.total_clicks,
...
waiting AS (
  SELECT * FROM waiting_raw
   ...
   ORDER BY (reason = 'replied, no response') DESC, last_inbound ASC
   LIMIT 200
),
pipeline AS (
...
totals AS (
  SELECT (SELECT count(*) FROM arrivals) AS arrivals,
         ...
         (SELECT count(*) FROM waiting)  AS waiting,
         ...
         (SELECT count(*) FROM stalled) AS stalled,
